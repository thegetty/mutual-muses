...
transcription_workflows <- c("28.76")
...
comb_df <- bind_rows(
  current = filter(processed_export, workflow_version == "28.76"),
  beta = filter(processed_export, workflow_version == "18.45"),
  .id = "phase"
)

class_df <- function(df = comb_df) {
  filter(df, phase == "current") %>% 
    filter(is_retired)
}

beta_df <- function(df = comb_df) {
  filter(df, phase == "beta")
}

n_empty <- sum(comb_df$charcount == 0)

comb_df <- comb_df %>% filter(charcount > 0)
```
...
```{r top_transcribers}
comb_df %>% 
  count(user_name, phase) %>% 
  ggplot(aes(x = n, fill = phase)) +
